...
public class TrackingOnly extends EventFilter2D implements FrameAnnotater {
    ...
    private int initPosX = getInt("initPosX",120); // Initial centreX coordinate of cluster
    private int initPosY = getInt("initPosY",90); // Initial centreY coordinate of cluster
    ...
    // TrackingOnly class constructor
    public TrackingOnly(AEChip chip) {
        super(chip);
        this.chip = chip;
    ...
    synchronized public void resetFilter() {
        ...
        log.warning("TrackingOnly has been reset");
    }
    ...
    public EventPacket<? extends BasicEvent> filterPacket(EventPacket<? extends BasicEvent> in) {
        ...
        if (weightSum > 0){
            ...
            updateCluster(dist,avgX,avgY,time,nEvPacket);
        }
        cluster.ts = time;
        resetPacketVars();
        return in;
    ...
    protected void updateCluster(double dist,int avgX,int avgY,int time, int nEvPacket) {
        ...
        if((nEvWindow < eventThreshold && cluster.windowRadius == cluster.minWindowRadius)||(nEvPacket < eventThreshold)){ // Detection
            cluster.processingMode = "Idle";
            cluster.velX = 0;
            cluster.velY = 0;
        }
        else{ // Tracking           
            ...
            //log.warning(String.format("%f, %f, %f", deltaT, cluster.velX,cluster.velY));
            
            if ((writeToFile) && (fileName != null)) {
                try (FileWriter myWriter = new FileWriter(fileName,true)) {
                    myWriter.write(Double.toString(cluster.centreX) +
                               ", " + Double.toString(cluster.centreY) + "\n");
                } catch (IOException e) {
    ...
    public class Window {
        ...
        public void draw(GLAutoDrawable drawable) {
            ...
            if ("Tracking".equals(cluster.processingMode)){
                gl.glPushMatrix();
                gl.glTranslatef(cluster.centreX, cluster.centreY, 0); // Center everything -> origin = 0
                ...
                gl.glPointSize(16f);
                gl.glBegin(GL.GL_POINTS);
                gl.glVertex2f(cluster.centreX, cluster.centreY); //Detection estimate
                gl.glEnd();
            }
    ...
    // <editor-fold defaultstate="collapsed" desc="getter-setter for --initPosX-">
    public int getinitPosX() {
        return initPosX;
    }
    public void setinitPosX(int initPosX) {
        if (initPosX < 0){initPosX = 0;}
        if (initPosX > chip.getSizeX()){initPosX = chip.getSizeX();}
        this.initPosX = initPosX;
        putInt("initPosX", initPosX);
    }
    // </editor-fold>
    
    // <editor-fold defaultstate="collapsed" desc="getter-setter for --initPosY-">
    public int getinitPosY() {
        return initPosY;
    }
    public void setinitPosY(int initPosY) {
        if (initPosY < 0){initPosY = 0;}
        if (initPosY > chip.getSizeY()){initPosY = chip.getSizeY();}
        this.initPosY = initPosY;
        putInt("initPosY", initPosY);
    }
    // </editor-fold>
